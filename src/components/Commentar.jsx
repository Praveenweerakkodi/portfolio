    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                ...
                if (error) {
                    console.debug('Note: Comments not available', error.message);
                    return;
                }

                const allComments = data || [];

                // Separate pinned and regular comments if is_pinned column exists
                const pinned = allComments.find(c => c.is_pinned === true) || null;
                const regular = allComments.filter(c => !c.is_pinned);

                setPinnedComment(pinned);
                setComments(regular);
            } catch (error) {
                console.debug('Error fetching comments:', error.message);
            }
        };
